...
# Country-export local verification — 2026-09-21

Public release verification also passed against https://szkocot.github.io/mivue-trafficcam/ after successful deployment 35622164123. Fresh Chromium used synthetic local records and actual published boundaries: reduced BIN reparsed, full backup byte-identical, linked endpoints acknowledged, unsafe unknown-link BIN blocked while GeoJSON remained available, notices retained, boundary body fetched once per worker session, Polish export controls present, private files 404 and zero page errors. No proprietary sample was uploaded. Local merged totals: 237 Node / 62 browser pass, one opt-in live-source skip.

Post-review verification: 237 Node tests and 62 Chromium tests pass; one opt-in live-source test is skipped. Added RED→GREEN regressions prove decision-only cancellation reuses completed classifications, edits invalidate them, and uncertain/linked rows display coordinates, country context, component IDs and triggering records. Review was independent but reused an existing reviewer because a fresh agent could not be spawned at the session thread limit. Minor filename-label polish is deferred. Both accepted-CANARD and no-CANARD builds pass; integration/public verification remains pending.

Nine focused real-worker UI cases cover immutable full backups, reparsed reduced BIN, notices, border review, bidirectional linked endpoints, corrupt-boundary retry, cancellation, A→B→A scope changes, edit/undo invalidation, late replies, Polish reopening, keyboard selection and 390 px layout. Real private-sample preview took 3,488 ms with 100 preview rows; cancellation followed by retry succeeded. The full private sample rebuilt byte-identically in 849 ms. These are local Chromium checks, not device or cross-browser acceptance; final review/public release checks remain pending.
