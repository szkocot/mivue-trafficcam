...
Implementation has not started. The user must review this plan and choose execution: subagent-driven task-by-task implementation/review, or native implementation in this session with a fresh whole-branch reviewer at the end. Native is recommended because the eight tasks share tightly coupled snapshot/provenance contracts and sequential access gates; retain independent final review before release.
