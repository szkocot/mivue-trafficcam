...
Current release status: user-authorized activation completed on 2026-09-20. A fresh bounded recheck accepted the reviewed terms/robots/schema and reproduced the exact candidate digest below. That candidate is published as data commit `7bb196e`, deployed and public-browser verified in run `35529021193`; both publication gates are enabled. Daily scheduled execution has not yet been observed. Historical pre-release notes below do not change this status or expand permission to scrape detail endpoints. Reuse safeguards remain an assessment, not legal certification.
