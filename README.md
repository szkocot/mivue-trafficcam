...
The next increment has an [approved automatic-ingestion design](docs/superpowers/specs/2026-09-20-source-ingestion-design.md): imports will merge supported additions/updates automatically while preserving manual edits and deletions, retaining attribution, and offering whole-import undo. The approved [ingestion plan](docs/superpowers/plans/2026-09-20-source-ingestion.md), including optional source-reported speed-limit/metadata display, remains unimplemented. The independent [browser-location plan](docs/superpowers/plans/2026-09-20-browser-location.md) is implemented and integrated into `main`. CANARD access/reuse verification and explicit binary-template safeguards remain required. Country-limited datasets and GitHub Pages deployment are follow-up tasks; deployment is authorized after the remaining work is complete and verified.

## Usage

Source-ingestion task 3 adds the tested reconciliation engine: stable source identities, repeat-import no-ops, 100 m duplicate suggestions without automatic identity matching, preserved manual coordinate groups/deletions, and explicit bind/reference/distinct decisions. Active supported points can clone only an explicitly configured original template; speed/direction metadata is never written into undocumented bytes. OPP remains reference-only. Browser integration is still pending.

Source-ingestion tasks 1–2 on `feat/source-ingestion` add tested local CSV/GeoJSON parsers and project v2 ingestion state with explicit manual-coordinate ownership and atomic undo transactions; project merging and browser import controls are not implemented yet. Existing v1 projects load and upgrade in memory; newly saved v2 projects require this release. CSV requires `id,latitude,longitude,kind`; optional fields are `name,speed,speed_unit,direction,status,url`, with extra columns retained as metadata. Coordinates use decimal dots; a supplied speed requires `km/h` or `mph`. Kinds: `camera`, `section`, `red-light`, `unknown`; status: `active`, `planned`, `inactive`, `unknown`. GeoJSON accepts identified Point features and section LineStrings in longitude/latitude order. Source namespace and attribution are required separately. Malformed batches are rejected in full; inputs are limited to 20 MiB/100,000 features, 1,000 vertices per section and 64 KiB metadata per feature. Neither parser guesses MiVue speed or direction fields.
