...
The [automatic-ingestion design](docs/superpowers/specs/2026-09-20-source-ingestion-design.md) is being implemented on `feat/source-ingestion`: local imports merge supported additions/updates while preserving manual edits/deletions, attribution and whole-import undo. Tasks 1–6 include PL/EN import controls and optional source-reported speed/metadata labels; final regression and independent review remain pending. The independent [browser-location plan](docs/superpowers/plans/2026-09-20-browser-location.md) is integrated into `main`. Country-limited datasets and GitHub Pages deployment are follow-up tasks; deployment is authorized after the remaining work is complete and verified.
