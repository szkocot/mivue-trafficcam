...
The next increment has an [approved automatic-ingestion design](docs/superpowers/specs/2026-09-20-source-ingestion-design.md): imports merge supported additions/updates automatically while preserving manual edits and deletions, retaining attribution, and offering whole-import undo. It also includes optional source-reported speed-limit/metadata display and an opt-in browser “Locate me” action with an accuracy circle and no saved location history. The [ingestion plan](docs/superpowers/plans/2026-09-20-source-ingestion.md) and independent [browser-location plan](docs/superpowers/plans/2026-09-20-browser-location.md) await review; these features are not implemented yet. CANARD access/reuse verification and explicit binary-template safeguards are included. Country-limited datasets and GitHub Pages deployment remain follow-up tasks, not completed features.
