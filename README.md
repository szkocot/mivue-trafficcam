...
The [automatic-ingestion design](docs/superpowers/specs/2026-09-20-source-ingestion-design.md) is implemented on `feat/source-ingestion`: local imports merge supported additions/updates while preserving manual edits/deletions, attribution and whole-import undo. Tasks 1–7 include PL/EN import controls, optional source-reported speed/metadata labels and regression coverage; independent review remains pending. The independent [browser-location plan](docs/superpowers/plans/2026-09-20-browser-location.md) is integrated into `main`. Country-limited datasets and GitHub Pages deployment are follow-up tasks; deployment is authorized after the remaining work is complete and verified.

Task 7 verification: **133 Node tests, 40 browser tests pass**, with the opt-in live-source check skipped. The synthetic 100,000-point import took 5.4 s in the observed run, with bounded table/label rendering; one extra observation is rejected. The full local sample still rebuilds byte-identically. Regression fixes cover repeat-policy no-op history, explicit same-value manual coordinates, inactive-source flags and source category changes. [Detailed verification](docs/browser-verification.md) records measurements and limitations; device acceptance remains untested.
