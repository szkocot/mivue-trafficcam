...
export function renderRecordList(element,{records,page,selectedId,onSelect,onPage,t}){
 ...
 for(const r of result.records){
  const row=body.insertRow();row.dataset.testid='record-row';if(r.id===selectedId)row.className='selected';
 ...
 for(const [key,delta] of [['previous',-1],['next',1]]){
  const b=document.createElement('button');b.textContent=t(key);b.disabled=delta<0?result.page===1:result.page===result.pageCount;
